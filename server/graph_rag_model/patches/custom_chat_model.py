    ...
    async def achat(
        ...
        if "response_format" in new_kwargs:
            ...
            try:
                ...
                if match:
                    try:
                        parsed_dict = json.loads(match.group(1).strip())
                    except json.JSONDecodeError:
                        logger.error(
                            "Failed to parse JSON even after extracting from code block. Raw: %s",
                            raw_content[:500],
                        )
                        parsed_dict = {}
                else:
                    logger.error(
                        "Content is not valid JSON and no code block found. Raw: %s",
                        raw_content[:500],
                    )
                    parsed_dict = {}
            ...
            if (
                    isinstance(new_kwargs["response_format"], type)
                    and issubclass(new_kwargs["response_format"], BaseModel)
            ):
                try:
                    model_initializer = cast("type[BaseModel]", new_kwargs["response_format"])
                    parsed_response = model_initializer(**parsed_dict)
                except Exception:
                    logger.exception("Failed to instantiate pydantic model from parsed JSON.")
                    # 保留字典形式作为 fallback
